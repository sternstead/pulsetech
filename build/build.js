...
/* --- static redirect stub pages ---
   _redirects and .htaccess only work on hosts that read them (Netlify,
   Cloudflare Pages, Apache). Hosts that just serve files as-is — GitHub
   Pages chief among them — need an actual HTML file sitting at the old
   URL. Skip any legacy path a real generated page already occupies
   (e.g. /index.html, /services/index.html): that file already serves
   correctly there and a stub would clobber it. */
const redirectStub = (to) => `<!-- redirect-stub: generated, not a content page, see qa.js -->
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
legacy.forEach(([from, to]) => {
  const dest = path.join(OUT, from.replace(/^\//, ""));
  if (writtenSet.has(dest)) return; // a real generated page already lives here
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  fs.writeFileSync(dest, redirectStub(to));
  stubCount++;
});

/* --- .nojekyll ---
   GitHub Pages runs a Jekyll build by default unless this file is
   present. Jekyll ignores any file/folder starting with an underscore
   (silently swallowing _redirects) and, if it ever can't find an
   index.html where it expects one, falls back to rendering README.md
   as the homepage. This file switches Pages to serve the folder as
   plain static files instead. */
...
console.log(`\nsitemap.xml, robots.txt, _redirects, .htaccess, ${stubCount} redirect stub pages, .nojekyll, favicon.svg written.`);
