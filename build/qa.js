...
files.forEach(f=>{
  ...
  // redirect stubs (old .html URLs -> clean URLs) are generated,
  // link-only pages, not content — only check the redirect target resolves
  if(h.startsWith('<!-- redirect-stub:')){
    const m=h.match(/meta http-equiv="refresh" content="0; url=([^"]+)"/);
    const to=m&&m[1];
    if(!to){issues.push(`${rel}: redirect stub missing refresh target`);return;}
    const clean=to.split('#')[0].split('?')[0];
    const target=clean.startsWith('/')?path.join(OUT,clean):path.resolve(path.dirname(f),clean);
    if(!fs.existsSync(target)&&!fs.existsSync(path.join(target,'index.html')))
      issues.push(`${rel}: redirect stub target missing -> ${to}`);
    return;
  }
  // one h1
